...
class SOGetPlayerEngagementsOfTeamTest extends SOClassTest {
    ...
    void testExecuteSuccess() throws Exception {
        ...
        when(dbb.getPlayerEngagementsOfTeam(t1)).thenReturn(pl2);

        Response response = so.execute(t1);

        verify(dbb, times(1)).getPlayerEngagementsOfTeam(t1);
    ...
    void testExecuteError() throws Exception {
        ...
        doThrow(new SQLException()).when(dbb).getPlayerEngagementsOfTeam(t1);
