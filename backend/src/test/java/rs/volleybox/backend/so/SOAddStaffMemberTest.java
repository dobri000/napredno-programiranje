...
class SOAddStaffMemberTest extends SOClassTest {
    ...
    void testExecuteSuccess() throws Exception {
        doNothing().when(dbb).addStaffMemberEngagement(any(StaffMemberEngagement.class));
        ...
        verify(dbb, times(1)).addStaffMemberEngagement(any(StaffMemberEngagement.class));
    ...
    void testExecuteError() throws Exception {
        doThrow(new SQLException()).when(dbb).addStaffMemberEngagement(any(StaffMemberEngagement.class));
        ...
        assertEquals("Staff member engagement not successfully added", (String) response.getObject());
    }
